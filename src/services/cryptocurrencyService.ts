...
export class CryptocurrencyService {
  ...
  static async getCryptocurrencyByCoinId(
    coinId: string
  ): Promise<Cryptocurrency | null> {
    return await prisma.cryptocurrency.findUnique({
      where: { coinId },
  ...
  static async updatePrices(): Promise<void> {
    try {
      ...
      const coinIds = cryptocurrencies.map(
        (crypto: Cryptocurrency) => crypto.coinId
      );
      const prices = await this.fetchPrices(coinIds);

      const updatePromises = cryptocurrencies.map(
