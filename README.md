...
### Current Architecture (Development/Testing)

The service currently uses a **single-service event-driven architecture** for development and testing purposes:
...
**Current Event Flow (Simulation)**:

1. **Alert Triggered**: Price condition met → Alert Service
2. **Event Emitted**: Alert Service → Event Emitter
3. **Event Handled**: Event Controller receives event
4. **Notifications**: Multiple notification types simulated (logs)

### Production Microservice Architecture

For production deployment, the event-driven design can be easily split into microservices:

```
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   Alert Service │    │   Event Bus     │    │ Notification    │
│                 │    │   (Redis/Rabbit │    │   Service       │
│ • Create Alerts │───▶│   MQ/Kafka)     │───▶│                 │
│ • Check Prices  │    │                 │    │ • Email         │
│ • Trigger Alerts│    │                 │    │ • Push          │
└─────────────────┘    └─────────────────┘    │ • SMS           │
                                              └─────────────────┘
